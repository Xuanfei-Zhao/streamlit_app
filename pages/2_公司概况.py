...
with st.spinner("加载财务排名数据..."):
    financial_rankings = get_company_financial_rankings(stock_code,year=2024)
...
if financial_rankings is not None:
    ...
    ranking_data = pd.DataFrame({
        ...
        '公司值': [
            safe_num(financial_rankings['roe']['value']),
            safe_num(financial_rankings['operating_margin']['value']),
            safe_num(financial_rankings['roa']['value']),
            safe_num(financial_rankings['ebitda_margin']['value']),
            safe_num(financial_rankings['asset_turnover']['value']),
            safe_num(financial_rankings['current_ratio']['value']),
            safe_num(financial_rankings['debt_ratio']['value'])
        ],
        '行业中位数': [
            safe_num(financial_rankings['roe']['median']),
            safe_num(financial_rankings['operating_margin']['median']),
            safe_num(financial_rankings['roa']['median']),
            safe_num(financial_rankings['ebitda_margin']['median']),
            safe_num(financial_rankings['asset_turnover']['median']),
            safe_num(financial_rankings['current_ratio']['median']),
            safe_num(financial_rankings['debt_ratio']['median'])
        ],
        '行业排名': [
            int(financial_rankings['roe']['rank']) if financial_rankings['roe']['rank'] is not None else '-',
            int(financial_rankings['operating_margin']['rank']) if financial_rankings['operating_margin']['rank'] is not None else '-',
            int(financial_rankings['roa']['rank']) if financial_rankings['roa']['rank'] is not None else '-',
            int(financial_rankings['ebitda_margin']['rank']) if financial_rankings['ebitda_margin']['rank'] is not None else '-',
            int(financial_rankings['asset_turnover']['rank']) if financial_rankings['asset_turnover']['rank'] is not None else '-',
            int(financial_rankings['current_ratio']['rank']) if financial_rankings['current_ratio']['rank'] is not None else '-',
            int(financial_rankings['debt_ratio']['rank']) if financial_rankings['debt_ratio']['rank'] is not None else '-'
        ],
        '百分位': [
            safe_pct(financial_rankings['roe']['percentile']),
            safe_pct(financial_rankings['operating_margin']['percentile']),
            safe_pct(financial_rankings['roa']['percentile']),
            safe_pct(financial_rankings['ebitda_margin']['percentile']),
            safe_pct(financial_rankings['asset_turnover']['percentile']),
            safe_pct(financial_rankings['current_ratio']['percentile']),
            safe_pct(financial_rankings['debt_ratio']['percentile'])
        ]
    })
...
tab1, = st.tabs(["综合维度雷达图"])   # 注意逗号表示解包元组
