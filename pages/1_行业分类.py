...
def load_company_keywords():
    """加载公司5年TOP20关键词数据"""
    df = pd.read_csv("company_5year_top20_tfidf_keywords_v2.csv", encoding='utf-8')
    # 统一股票代码格式为6位字符串（如果存在）
...
def load_cross_industry():
    file_path = Path("stage17_final_cross_industry_mapping_table.xlsx")
    if not file_path.exists():
...
st.divider()
st.subheader("🏢 公司 TOP20 关键词权重分布")
def plot_company_top20_keywords(stock_code):
    """
    根据股票代码绘制公司TOP20关键词权重水平条形图
    返回 plotly Figure 对象，可直接用于 st.plotly_chart
    """
    df = load_company_keywords()
    # 筛选当前公司数据，按 rank 升序排列（1 权重最高）
    company_df = df[df['stock_code'] == stock_code].copy()
    if company_df.empty:
        return None
    
    company_df = company_df.sort_values('rank')  # rank 1 到 20
    # 确保关键词不重复（理论上不会重复，但为了安全）
    keywords = company_df['term'].tolist()
    weights = company_df['tfidf_weight'].tolist()
    ...
    fig.update_layout(
        xaxis_title="TF-IDF 权重",
        yaxis_title="关键词",
        yaxis=dict(categoryorder='total ascending'),  # 按权重从低到高显示（最重在上）
        height=500,
        margin=dict(l=0, r=0, t=50, b=0)
    )
    return fig

# 在页面中使用示例（假设已有 company_code_norm）
fig = plot_company_top20_keywords(company_code_norm)
if fig:
    st.plotly_chart(fig, use_container_width=True)
else:
    st.info("未找到该公司的关键词数据")
