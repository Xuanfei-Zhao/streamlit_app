...
def load_company_mapping():
    try:
        ...
        try:
            df = pd.read_excel("complete_company_industry_mapping_v5_qwen_level1_final.xlsx", engine="xlrd")
...
def get_comprehensive_radar_data(dimensions_data):
    """回退方案：从各维度数据中取每个维度的第一个指标的公司值作为维度得分"""
...
# 3. 综合雷达图（优先使用综合JSON）
st.markdown("## 综合财务雷达图")
comp_json = load_comprehensive_json()
comp_dims, comp_scores = None, None
if comp_json:
    key = find_company_key(comp_json, selected_code, selected_name)
    if key and 'dimensions' in comp_json[key] and 'scores' in comp_json[key]:
        comp_dims = comp_json[key]['dimensions']
        comp_scores = comp_json[key]['scores']
if comp_dims and comp_scores:
    fig_comp = create_radar_chart(comp_dims, comp_scores, title="公司各维度综合得分", show_benchmark=False)
    st.plotly_chart(fig_comp, use_container_width=True)
else:
    # 回退：从各维度数据取第一个指标得分
    dim_names, fallback_scores = get_comprehensive_radar_data(dimensions_data)
    if dim_names:
        fig_comp = create_radar_chart(dim_names, fallback_scores, title="公司各维度综合得分（备用）", show_benchmark=False)
        st.plotly_chart(fig_comp, use_container_width=True)
    else:
...
# 4. 条形图：2024年 vs 5年均值（使用“公司各纬度得分.csv”）
    ...
    with st.expander(f"📈 {dim}分析", expanded=(dim == "盈利能力指标" or dim == "盈利能力")):
        ...
        with col_right:
            ...
            for i, ind in enumerate(indicators):
                # 公司值：保留五位小数
                company_display = f"{company_vals[i]:.5f}" if not pd.isna(company_vals[i]) else "N/A"
                # 行业中位数：保留五位小数
                industry_display = f"{industry_vals[i]:.5f}" if not pd.isna(industry_vals[i]) else "N/A"
                # 分位数：保留三位小数
...
st.caption("数据来源：公司各维度雷达图JSON + 新数据源(raw) + 各维度得分CSV + 年度财务数据。公司值/行业中位数保留五位小数，分位数保留三位小数，行业排名为整数。")