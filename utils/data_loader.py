...
def get_company_financial_rankings(stock_code, year=None):
    """
    获取公司的财务指标排名数据
    返回包含ROE、营业利润率等关键指标的排名信息，所有数值已转为 float/int，None 表示缺失
    """
    stats_df = load_company_statistics()
    stock_code_str = str(stock_code).zfill(6)  # 标准化代码

    # 匹配股票代码（假设 CSV 中有 stock_code_norm 列）
    filtered = stats_df[stats_df['stock_code_norm'].astype(str).str.zfill(6) == stock_code_str]
